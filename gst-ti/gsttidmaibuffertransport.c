...
static void gst_tidmaibuffertransport_finalize(GstBuffer *gstbuffer)
{
    ...
    if (Buffer_getBufTab(self->dmaiBuffer) != NULL) {
        ...
        Buffer_freeUseMask(self->dmaiBuffer, gst_tidmaibuffer_VIDEOSRC_FREE);
    } else {
        GST_LOG("calling Buffer_delete()\n");
        Buffer_delete(self->dmaiBuffer);
    }

    /* If element has registered its finalize callback then execute it */
    if (self->callback) {
        self->callback(self->callback_args, gstbuffer);
    }
 ...
 ******************************************************************************/
GstBuffer* gst_tidmaibuffertransport_new(Buffer_Handle dmaiBuffer, 
 GstTIDmaiBufTab *owner, void (*callback) (void *args, GstBuffer *buf), void *args)
{
    ...
    tdt_buf->owner      = owner;
    tdt_buf->callback   = callback;
    tdt_buf->callback_args = args;
