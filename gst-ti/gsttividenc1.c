...
static gboolean gst_tividenc1_codec_start (GstTIVidenc1 *videnc1)
{
    ...
    GST_LOG("configuring video encode width=%ld, height=%ld, bitrate=%ld\n", 
		...
		/* extend the params and dynParams 20110520 */
		IH264VENC_DynamicParams extDynParams =
		{
			...
			0, // maxMBsPerSlice
			0, // maxBytesPerSlice
			1, // sliceRefreshRowStartNumber
			0, // sliceRefreshRowNumber
			0, // filterOffsetA
			0, // filterOffsetB
			0, // log2MaxFNumMinus4
			0, // chromaQPIndexOffset
			0, // constrainedIntraPredEnable
			0, // picOrderCountType
			1, // maxMVperMB
			INTRA4x4_NONE,// intra4x4EnableIdc
			0,// mvDataEnable
			1,// hierCodingEnable
			IH264_BYTE_STREAM,// streamFormat
			IH264_INTRAREFRESH_NONE,// intraRefreshMethod
			0,// perceptualQuant
			1,// sceneChangeDet
			NULL,//
			NULL,//
		...
		IH264VENC_Params extParams = {
			...
			66,                                                  // profileIdc
			IH264_LEVEL_30,                       // levelIdc
			0,
			16,
		};

		extParams.videncParams.size                                         = sizeof(IH264VENC_Params);
		extParams.videncParams.rateControlPreset    = IVIDEO_NONE;
		extParams.videncParams.maxBitRate                 = 2000;
		extParams.videncParams.inputChromaFormat   = XDM_YUV_422ILE;
		extParams.videncParams.maxWidth                    = 720;  //352
		extParams.videncParams.maxHeight                  = 576;  //288
		...
		extDynParams.videncDynamicParams.intraFrameInterval = 200;
    ...
    GstBuffer **outBuf)
{
    ...
    *outBuf = NULL;
    ...
    gst_tividenc1_set_source_caps(videnc1, videnc1->hEncOutBuf);
		...
		char *src = (char *)Buffer_getUserPtr(videnc1->hEncOutBuf);
		int length = Buffer_getNumBytesUsed(videnc1->hEncOutBuf);
		static int fn;
		int spslen = 34;
		static char sps[128];
     ...
     */
    	...
    	*outBuf = gst_buffer_new_and_alloc(Buffer_getNumBytesUsed(videnc1->hEncOutBuf) + spslen);
			memcpy(GST_BUFFER_DATA(*outBuf), sps, spslen);
	    memcpy(((char *)GST_BUFFER_DATA(*outBuf) + spslen), Buffer_getUserPtr(videnc1->hEncOutBuf),
        Buffer_getNumBytesUsed(videnc1->hEncOutBuf));
		}
